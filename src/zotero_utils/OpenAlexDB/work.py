...
class Work:
    ...
    @staticmethod
    def load_works_from_db_by_ids(conn: sqlite3.Connection, work_ids: Union[List[str], str]) ->  "Work":
        """
    ...
    @staticmethod
    def insert_works_from_web_api_by_ids(conn: sqlite3.Connection, work_ids: Union[List[str], str]) -> "Work":
        """
    ...
    def insert_into_db(self, conn: sqlite3.Connection):
        """
        ...
        conn.execute(
            f"INSERT INTO works (id, doi, title, display_name, publication_year, publication_date, type, cited_by_count, is_retracted, is_paratext, cited_by_api_url, abstract_inverted_index, language) VALUES ({question_marks})", insert_tuple
        )
        ...
        question_marks = ', '.join(['?'] * len(insert_tuple))
        conn.execute(
            f"INSERT INTO works_primary_locations (work_id, source_id, landing_page_url, pdf_url, is_oa, version, license) VALUES ({question_marks})", insert_tuple
        )
        ...
        for location in work['locations']:
            ...
            question_marks = ', '.join(['?'] * len(insert_tuple))
            conn.execute(
                f"INSERT INTO works_locations (work_id, source_id, landing_page_url, pdf_url, is_oa, version, license) VALUES ({question_marks})", insert_tuple
            )
        ...
        question_marks = ', '.join(['?'] * len(insert_tuple))
        conn.execute(
            f"INSERT INTO works_best_oa_locations (work_id, source_id, landing_page_url, pdf_url, is_oa, version, license) VALUES ({question_marks})", insert_tuple
        )
        ...
        for authorship in work['authorships']:
            for institution in authorship['institutions']:
                ...
                question_marks = ', '.join(['?'] * len(insert_tuple))
                conn.execute(
                    f"INSERT INTO works_authorships (work_id, author_position, author_id, institution_id) VALUES ({question_marks})", insert_tuple
                )
        ...
        question_marks = ', '.join(['?'] * len(insert_tuple))
        conn.execute(
            f"INSERT INTO works_biblio (work_id, volume, issue, first_page, last_page) VALUES ({question_marks})", insert_tuple
        )
        ...
        for topic in work['topics']:
            ...
            question_marks = ', '.join(['?'] * len(insert_tuple))
            conn.execute(
                f"INSERT INTO works_topics (work_id, topic_id, score) VALUES ({question_marks})", insert_tuple
            )
        ...
        for concept in work['concepts']:
            ...
            question_marks = ', '.join(['?'] * len(insert_tuple))
            conn.execute(
                f"INSERT INTO works_concepts (work_id, concept_id, score) VALUES ({question_marks})", insert_tuple
            )
        ...
        question_marks = ', '.join(['?'] * len(insert_tuple))
        conn.execute(
            f"INSERT INTO works_ids (work_id, openalex, doi, mag, pmid, pmcid) VALUES ({question_marks})", insert_tuple
        )
        ...
        for mesh in work['mesh']:
            ...
            question_marks = ', '.join(['?'] * len(insert_tuple))
            conn.execute(
                f"INSERT INTO works_mesh (work_id, descriptor_ui, descriptor_name, qualifier_ui, qualifier_name, is_major_topic) VALUES ({question_marks})", insert_tuple
            )
        ...
        question_marks = ', '.join(['?'] * len(insert_tuple))
        conn.execute(
            f"INSERT INTO works_open_access (work_id, is_oa, oa_status, oa_url, any_repository_has_fulltext) VALUES ({question_marks})", insert_tuple
        )
        ...
        for referenced_work_id in work['referenced_works']:
            ...
            question_marks = ', '.join(['?'] * len(insert_tuple))            
            conn.execute(
                f"INSERT INTO works_referenced_works (work_id, referenced_work_id) VALUES ({question_marks})", insert_tuple
            )
        ...
        for related_work_id in work['related_works']:
            ...
            question_marks = ', '.join(['?'] * len(insert_tuple))
            conn.execute(
                f"INSERT INTO works_related_works (work_id, related_work_id) VALUES ({question_marks})", insert_tuple
            )
